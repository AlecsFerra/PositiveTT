...
def Models (ρ ρ' : DEnv n) (Γ : Ctx n) : Prop :=
  ∀ {i : Fin n} {τ : Tm n}, (Γ ∋ i ∶ τ) →
    ∃ ℓ X, Decode ℓ (⟦ τ ⟧𝒄 ρ) (⟦ τ ⟧𝒄 ρ') X ∧ X (ρ.get i) (ρ'.get i)
...
theorem Models.cons {ρ ρ' : DEnv n} {Γ : Ctx n} (hρ : ρ ∼ ρ' ⊨ Γ) {τ : Tm n} {ℓ : Nat}
    {A : D → D → Prop} (hA : Decode ℓ (⟦ τ ⟧𝒄 ρ) (⟦ τ ⟧𝒄 ρ') A)
    {d d' : D} (hd : A d d') : (ρ ∷ d) ∼ (ρ' ∷ d') ⊨ (Γ ∷ τ) := by
  intro i σ hlook
...
environments yields related values in the decoding of its type. In particular
functions are identified extensionally. -/
theorem WfTm.sound : ∀ {n : Nat} {Γ : Ctx n} {t τ : Tm n}, (Γ ⊢ t ∶ τ) →
    ∀ {ρ ρ' : DEnv n}, (ρ ∼ ρ' ⊨ Γ) →
    ∃ ℓ X, Decode ℓ (⟦ τ ⟧𝒄 ρ) (⟦ τ ⟧𝒄 ρ') X ∧ X (⟦ t ⟧𝒄 ρ) (⟦ t ⟧𝒄 ρ')
  ...
  | _, Γ, .app t s, _, .app ht hs, ρ, ρ', hρ => by
    ...
    · simpa [Tm.eval] using hmemt hmems
