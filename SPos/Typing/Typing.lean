import SPos.Syntax.Syntax
import Aesop

abbrev Ctx := Env Tm

section
set_option hygiene false
notation:40 "⊢" Γ:40 => WfCtx Γ
notation:40 Γ:41 "∋" x:41 "∶" τ:41 => Lookup Γ x τ
notation:40 Γ:41 "⊢" t:41 "≡" t':41 "∶" τ:41 => DefEq Γ t t' τ
notation:40 Γ:41 "⊢" t:41 "∶" τ:41 => DefEq Γ t t τ

inductive Lookup : Ctx n → Fin n → Tm n → Prop where
| here  : Γ ∷ τ ∋ 0 ∶ ↑ τ
| there : Γ ∋ n ∶ τ → Γ ∷ υ ∋ n.succ ∶ ↑ τ

mutual
inductive WfCtx : Ctx n → Prop where
| nil  : ⊢ ∅
| cons : ⊢ Γ → Γ ⊢ τ ∶ 𝓤 ℓ → ⊢ Γ ∷ τ

-- The idea is absuing structural rules to get the standard typing
-- judgjments

inductive DefEq : Ctx n → Tm n → Tm n → Tm n → Prop where
-- Structual rules
| symm  : Γ ⊢ t₁ ≡ t₂ ∶ τ
        -----------------
       → Γ ⊢ t₂ ≡ t₁ ∶ τ
| trans : Γ ⊢ t₁ ≡ t₂ ∶ τ → Γ ⊢ t₂ ≡ t₃ ∶ τ
        ------------------------------------
       → Γ ⊢ t₁ ≡ t₃ ∶ τ
-- Conversion
| conv : Γ ⊢ t₁ ≡ t₂ ∶ τ₁ → Γ ⊢ τ₂ ∶ 𝓤 ℓ → Γ ⊢ τ₁ ≡ τ₂ ∶ 𝓤 ℓ
       --------------------------------------------------------
      → Γ ⊢ t₁ ≡ t₂ ∶ τ₂
-- Variable
| var : ⊢ Γ → Γ ∋ x ∶ τ
      ------------------
     → Γ ⊢ # x ∶ τ
-- Universe
| u : ⊢ Γ
    ----------------------
   → Γ ⊢ 𝓤 ℓ ∶ 𝓤 (ℓ + 1)
-- Pi
| pi  : Γ ⊢ τ₁ ≡ τ₂ ∶ 𝓤 ℓ₁ → Γ ∷ τ₁ ⊢ υ₁ ≡ υ₂ ∶ 𝓤 ℓ₂
      ------------------------------------------------
     → Γ ⊢ Π τ₁ υ₁ ≡ Π τ₂ υ₂ ∶ 𝓤 (max ℓ₁ ℓ₂)
-- Alecs(TODO): Figure out how to drop the well formedness of υ
| lam : Γ ∷ τ₁ ⊢ υ ∶ 𝓤 ℓ' → Γ ⊢ τ₁ ≡ τ₂ ∶ 𝓤 ℓ → Γ ∷ τ₁ ⊢ t₁ ≡ t₂ ∶ υ
      ----------------------------------------------------------------
     → Γ ⊢ ƛ τ₁ t₁ ≡ ƛ τ₂ t₂ ∶ (Π τ₁ υ)
| app : Γ ⊢ t₁ ≡ t₂ ∶ (Π τ υ) → Γ ⊢ m₁ ≡ m₂ ∶ τ
      ------------------------------------------
     → Γ ⊢ t₁ • m₁ ≡ t₂ • m₂ ∶ υ [/ m₁ ]
| lamβ : Γ ⊢ τ ∶ 𝓤 ℓ → Γ ∷ τ ⊢ υ ∶ 𝓤 ℓ' → Γ ∷ τ ⊢ t ∶ υ → Γ ⊢ m ∶ τ
       -----------------------------------------------------------------
      → Γ ⊢ (ƛ τ t) • m ≡ t [/ m ] ∶ υ [/ m ]
| lamη : Γ ⊢ t ∶ Π τ υ
       -----------------------------------
      → Γ ⊢ t ≡ ƛ τ (↑ t) • # 0 ∶ Π τ υ
-- Sigma
| sigma : Γ ⊢ τ₁ ≡ τ₂ ∶ 𝓤 ℓ₁ → Γ ∷ τ₁ ⊢ υ₁ ≡ υ₂ ∶ 𝓤 ℓ₂
        ------------------------------------------------------
       → Γ ⊢ Σ̶ τ₁ υ₁ ≡ Σ̶ τ₂ υ₂ ∶ 𝓤 (max ℓ₁ ℓ₂)
| pair : Γ ⊢ τ ∶ 𝓤 ℓ → Γ ∷ τ ⊢ υ ∶ 𝓤 ℓ' → Γ ⊢ a₁ ≡ a₂ ∶ τ
      → Γ ⊢ b₁ ≡ b₂ ∶ υ [/ a₁ ]
       ------------------------------------------------------
      → Γ ⊢ ⸨ a₁ , b₁ ⸩ ≡ ⸨ a₂ , b₂ ⸩ ∶ Σ̶ τ υ
| fst : Γ ⊢ p₁ ≡ p₂ ∶ Σ̶ τ υ
      -------------------------------
     → Γ ⊢ pr₁ p₁ ≡ pr₁ p₂ ∶ τ
| snd : Γ ⊢ p₁ ≡ p₂ ∶ Σ̶ τ υ
      ----------------------------------------------
     → Γ ⊢ pr₂ p₁ ≡ pr₂ p₂ ∶ υ [/ pr₁ p₁ ]
| fstβ : Γ ⊢ τ ∶ 𝓤 ℓ → Γ ∷ τ ⊢ υ ∶ 𝓤 ℓ' → Γ ⊢ a ∶ τ → Γ ⊢ b ∶ υ [/ a ]
       -------------------------------------------------------------------
      → Γ ⊢ pr₁ ⸨ a , b ⸩ ≡ a ∶ τ
| sndβ : Γ ⊢ τ ∶ 𝓤 ℓ → Γ ∷ τ ⊢ υ ∶ 𝓤 ℓ' → Γ ⊢ a ∶ τ → Γ ⊢ b ∶ υ [/ a ]
       -------------------------------------------------------------------
      → Γ ⊢ pr₂ ⸨ a , b ⸩ ≡ b ∶ υ [/ a ]
| pairη : Γ ⊢ p ∶ Σ̶ τ υ
        ------------------------------------
       → Γ ⊢ p ≡ ⸨ pr₁ p , pr₂ p ⸩ ∶ Σ̶ τ υ
-- Booleans
| bool : ⊢ Γ
       ---------------------
      → Γ ⊢ Tm.bool ∶ 𝓤 0
| true : ⊢ Γ
       -----------------------
      → Γ ⊢ Tm.true ∶ Tm.bool
| false : ⊢ Γ
        -------------------------
       → Γ ⊢ Tm.false ∶ Tm.bool
| boolrec : Γ ∷ Tm.bool ⊢ P₁ ≡ P₂ ∶ 𝓤 ℓ
          → Γ ⊢ t₁ ≡ t₂ ∶ P₁ [/ Tm.true ] → Γ ⊢ f₁ ≡ f₂ ∶ P₁ [/ Tm.false ]
          → Γ ⊢ b₁ ≡ b₂ ∶ Tm.bool
          ------------------------------------------------------------------
          → Γ ⊢ Tm.boolrec P₁ t₁ f₁ b₁ ≡ Tm.boolrec P₂ t₂ f₂ b₂ ∶ P₁ [/ b₁ ]
| boolβt : Γ ∷ Tm.bool ⊢ P ∶ 𝓤 ℓ
         → Γ ⊢ t ∶ P [/ Tm.true ] → Γ ⊢ f ∶ P [/ Tm.false ]
         --------------------------------------------------------
         → Γ ⊢ Tm.boolrec P t f Tm.true ≡ t ∶ P [/ Tm.true ]
| boolβf : Γ ∷ Tm.bool ⊢ P ∶ 𝓤 ℓ
         → Γ ⊢ t ∶ P [/ Tm.true ] → Γ ⊢ f ∶ P [/ Tm.false ]
         --------------------------------------------------------
         → Γ ⊢ Tm.boolrec P t f Tm.false ≡ f ∶ P [/ Tm.false ]
-- Identity
| id : Γ ⊢ τ₁ ≡ τ₂ ∶ 𝓤 ℓ → Γ ⊢ a₁ ≡ a₂ ∶ τ₁ → Γ ⊢ b₁ ≡ b₂ ∶ τ₁
     ------------------------------------------------------------
    → Γ ⊢ Id τ₁ a₁ b₁ ≡ Id τ₂ a₂ b₂ ∶ 𝓤 ℓ
| refl : Γ ⊢ τ₁ ≡ τ₂ ∶ 𝓤 ℓ → Γ ⊢ a₁ ≡ a₂ ∶ τ₁
       -------------------------------------------
      → Γ ⊢ refl τ₁ a₁ ≡ refl τ₂ a₂ ∶ Id τ₁ a₁ a₁
| j : {a : Tm n} → Γ ⊢ τ ∶ 𝓤 ℓ₁ → Γ ∷ τ ⊢ Id (↑ τ) (↑ a) # 0 ∶ 𝓤 ℓ₁
   → Γ ∷ τ ∷ Id (↑ τ) (↑ a) # 0 ⊢ C₁ ≡ C₂ ∶ 𝓤 ℓ
   → Γ ⊢ d₁ ≡ d₂ ∶ C₁ [/ refl (↑ τ) (↑ a) ] [/ a ] → Γ ⊢ p₁ ≡ p₂ ∶ Id τ a b
    ----------------------------------------------------------------------------
   → Γ ⊢ J C₁ d₁ p₁ ≡ J C₂ d₂ p₂ ∶ C₁ [/ ↑ p₁ ] [/ b ]
| jβ : Γ ⊢ τ ∶ 𝓤 ℓ₁ → Γ ⊢ a ∶ τ →  Γ ∷ τ ⊢ Id (↑ τ) (↑ a) # 0 ∶ 𝓤 ℓ₁
    →  Γ ∷ τ ∷ Id (↑ τ) (↑ a) # 0 ⊢ C ∶ 𝓤 ℓ
    →  Γ ⊢ d ∶ C [/ refl (↑ τ) (↑ a) ] [/ a ]
     --------------------------------------------------------------
    →  Γ ⊢ J C d (refl τ a) ≡ d ∶ C [/ refl (↑ τ) (↑ a) ] [/ a ]
end
end

@[grind →]
theorem DefEq.wf_left (h : Γ ⊢ t₁ ≡ t₂ ∶ τ) : Γ ⊢ t₁ ∶ τ :=
  h.trans h.symm

@[grind →]
theorem DefEq.wf_right (h : Γ ⊢ t₁ ≡ t₂ ∶ τ) : Γ ⊢ t₂ ∶ τ :=
  h.symm.trans h

@[grind →]
theorem DefEq.convert (h : Γ ⊢ t₁ ≡ t₂ ∶ τ₁) (hτ : Γ ⊢ τ₂ ∶ 𝓤 ℓ)
  (hτσ : Γ ⊢ τ₁ ≡ τ₂ ∶ 𝓤 ℓ) : Γ ⊢ t₁ ≡ t₂ ∶ τ₂ := DefEq.conv h hτ hτσ
