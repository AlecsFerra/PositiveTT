...
| var : ⊢ Γ → Γ ∋ x ∶ τ
              → Γ ⊢ # x ∶ τ
| conv : Γ ⊢ t ∶ τ → Γ ⊢ σ ∶ 𝓤 ℓ → Γ ⊢ τ ≡ σ ∶ 𝓤 ℓ
                   → Γ ⊢ t ∶ σ
-- Pi
| pi  : Γ ⊢ τ ∶ 𝓤 ℓ₁ → Γ ∷ τ ⊢ υ ∶ 𝓤 ℓ₂
                    → Γ ⊢ Π τ υ ∶ 𝓤 (max ℓ₁ ℓ₂)
| lam : Γ ⊢ τ ∶ 𝓤 ℓ → Γ ∷ τ ⊢ σ ∶ 𝓤 ℓ' → Γ ∷ τ ⊢ t ∶ σ
                     → Γ ⊢ ƛ τ t ∶ (Π τ σ)
| app : Γ ⊢ t ∶ (Π τ σ) → Γ ⊢ m ∶ τ
                  → Γ ⊢ t • m ∶ σ [/ m ]
-- Universe
...
-- Typed conversion: `Γ ⊢ t ≡ t' ∶ τ` — `t` and `t'` are interchangeable at type `τ`.
...
| β     : Γ ⊢ ƛ τ t ∶ (Π τ σ) → Γ ⊢ m ∶ τ
                       → Γ ⊢ (ƛ τ t) • m ≡ t [/ m ] ∶ σ [/ m ]
| eta   : {n : Nat} → {Γ : Ctx n} → {t τ : Tm n} → {υ : Tm (n + 1)} →
    Γ ⊢ t ∶ (Π τ υ) →
    Γ ⊢ t ≡ ƛ τ (Tm.app t.weaken (Tm.var (0 : Fin (n + 1)))) ∶ (Π τ υ)
end
end
